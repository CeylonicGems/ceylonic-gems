  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Only seller accounts can request gemstone deletion.",
        },
        { status: 403 }
      );
    }
    ...
    if (profile.verification_status !== "verified") {
      return NextResponse.json(
        {
          error:
            "Your seller account must be verified before requesting deletion.",
        },
        { status: 403 }
      );
    }
    ...
    if (auditError) {
      console.error(
        "Deletion request audit log failed:",
        auditError
      );
    }
    ...
    console.error("Seller deletion request error:", error);
